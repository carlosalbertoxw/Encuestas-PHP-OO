...
class User_MDL {
    ...
    public function u_m_update_user($a) {
        $conn = $this->db->open_connection();
        $res = $this->db->execute_query($conn, "UPDATE a_users_profiles SET u_p_user='" . $conn->escape_string($a['user']) . "' WHERE u_p_key=" . $conn->escape_string($a['key']));
        $this->db->close_connection($conn);
        return $res;
    }
    ...
    public function u_m_get_profile($user) {
        $conn = $this->db->open_connection();
        $res = $this->db->get_result($conn, "SELECT * FROM a_users_profiles WHERE u_p_user='" . $conn->escape_string($user) . "'");
        $this->db->close_connection($conn);
        return $res;
    }
    ...
    public function u_m_get_user_by_email($a) {
        $conn = $this->db->open_connection();
        $res = $this->db->get_result($conn, "SELECT u.u_key,u.u_email,u.u_password,up.u_p_user,up.u_p_name FROM a_users as u JOIN a_users_profiles as up ON u.u_key=up.u_p_key WHERE u.u_email='" . $conn->escape_string($a['email']) . "'");
        $this->db->close_connection($conn);
        return $res;
    }
    ...
    public function u_m_sign_up($a) {
        ...
        $r2 = $this->db->execute_query($conn, "INSERT INTO a_users_profiles(u_p_key,u_p_user,u_p_name) VALUES((SELECT u_key FROM a_users WHERE u_email='" . $conn->escape_string($a['email']) . "'),concat('usuario',(SELECT u_key FROM a_users WHERE u_email='" . $conn->escape_string($a['email']) . "')),concat('Usuario',(SELECT u_key FROM a_users WHERE u_email='" . $conn->escape_string($a['email']) . "')))");
        setcookie("AAA", $r1 . '-' . $r2);
        if ($r1 == 0 and $r2 == 0) {
            $conn->commit();
            $res = 0;
