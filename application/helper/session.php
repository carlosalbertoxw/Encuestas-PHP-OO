...
function create_session_cookie($value, $remember = FALSE) {
    $cookie_name = 'AOAAOA';
    if ($remember == 'on') {
        setcookie($cookie_name, serialize($value), time() + 60 * 60 * 24 * 365, WEB_PATH, WEB_SITE_DOMAIN, RELEASED, RELEASED);
    } else {
        setcookie($cookie_name, serialize($value), 0, WEB_PATH, WEB_SITE_DOMAIN, RELEASED, RELEASED);
    }
}
